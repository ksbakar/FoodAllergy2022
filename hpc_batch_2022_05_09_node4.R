...
for(i in 1:length(delta)){
  for(j in 1:nrow(n_theta)){
    for(jj in 1:nrow(n_theta)){
      for(jjj in 1:nrow(n_theta)){
        out <- pblapply(1:nSim, function(x) trial_betabino(n=120,rand_assignment=c(1,1,1),rand_pr=c(0.33,0.33,0.33),
                                                           n_theta=c(n_theta[j,1],n_theta[jj,2],n_theta[jjj,3]),
                                                           delta=delta[i]))
        ...
        res_mat <- rbind(res_mat,cbind(1:3,id_scenario,40,delta[i],c(n_theta[j,1],n_theta[jj,2],n_theta[jjj,3]),est_theta,est_theta_sd,decision_pr))
	  }
    }
  }
}
...
##
write.csv(res_mat,file=paste0("result_40_arm3_prior1_",Sys.Date(),".csv"),row.names=FALSE)
##
