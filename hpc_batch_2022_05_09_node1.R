...
for(i in 1:length(delta)){
  for(j in 1:nrow(n_theta)){
    for(jj in 1:nrow(n_theta)){
        out <- pblapply(1:nSim, function(x) trial_betabino(n=90,rand_assignment=c(1,1),rand_pr=c(0.5,0.5),
        ...
        res_mat <- rbind(res_mat,cbind(1:2,id_scenario,45,delta[i],c(n_theta[j,1],n_theta[jj,2]),est_theta,est_theta_sd,decision_pr))
    }
  }
}
...
##
write.csv(res_mat,file=paste0("result_45_arm2_prior1_",Sys.Date(),".csv"),row.names=FALSE)
##
